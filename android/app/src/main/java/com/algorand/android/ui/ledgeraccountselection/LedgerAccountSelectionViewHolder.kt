...
class LedgerAccountSelectionViewHolder(
    ...
    override fun bind(item: AccountSelectionListItem) {
        ...
        with(binding) {
            with(item) {
                headerIconView.setImageResource(selectorDrawableRes)
                nameTextView.text = account.address.toShortenedAddress()
                setupAssetCount(assetInformationList.count())
                bindSelection(isSelected)
                infoButton.setOnClickListener { listener.onAccountInfoClick(this) }
                root.setOnClickListener { listener.onAccountItemClick(this) }
            }
        }
    }
