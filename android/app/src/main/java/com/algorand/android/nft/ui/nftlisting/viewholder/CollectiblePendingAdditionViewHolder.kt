...
class CollectiblePendingAdditionViewHolder(
    ...
    override fun bind(item: BaseCollectibleListItem.BaseCollectibleItem) {
        super.bind(item)
        if (item !is BaseCollectibleListItem.BaseCollectibleItem.BasePendingCollectibleItem.PendingAdditionItem) return
        loadPreview(item)
    }

    private fun loadPreview(
        item: BaseCollectibleListItem.BaseCollectibleItem.BasePendingCollectibleItem.PendingAdditionItem
    ) {
        with(binding) {
    ...
    companion object : NftListViewHolderCreator {
        override fun create(parent: ViewGroup): BaseCollectibleListViewHolder {
            return CollectiblePendingAdditionViewHolder(createItemNftListBinding(parent))
        }
    }
}
