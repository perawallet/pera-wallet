...
class NameServiceMapper @Inject constructor(
    private val nameServiceSourceMapper: NameServiceSourceMapper
) {

    fun mapToNameService(nameServiceDTO: NameServiceDTO): NameService {
        return with(nameServiceDTO) {
            NameService(
                accountAddress = accountAddress,
                nameServiceName = nameServiceName,
                nameServiceSource = nameServiceSourceMapper.mapToNameServiceSource(nameServiceSource),
                nameServiceUri = nameServiceUri
            )
        }
    }
}
