...
class PendingAssetViewHolder(
    ...
    fun bind(assetItem: AccountDetailAssetsItem.BaseAssetItem.BasePendingAssetItem) {
        with(binding) {
            with(assetItem) {
                ...
                assetNameTextView.apply {
                    setupUI(isVerified, formattedShortName, formattedFullName, assetItem.id, isAlgo)
                    showProgressBar()
                }
            }
        }
