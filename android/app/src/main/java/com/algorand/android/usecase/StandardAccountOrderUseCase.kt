...
class StandardAccountOrderUseCase @Inject constructor(
    ...
    override val defaultAccountType: Account.Type
        get() = Account.Type.STANDARD

    override fun getFilteredAccounts(): List<Account> {
        return accountManager.getAccounts().filterNot { it.type == Account.Type.WATCH }
    }
    ...
    override fun saveAccountsWithSelectedOrder(accountOrderList: List<AccountOrderItem>) {
        ...
        val orderedStandardAccountList = accountOrderList.mapIndexedNotNull { index, accountOrderItem ->
            standardAccounts.firstOrNull { it.address == accountOrderItem.publicKey }
                ?.copy(index = index + STANDARD_ACCOUNT_START_INDEX)
        }
        accountManager.saveAccounts(orderedStandardAccountList + watchAccounts)
    }

    companion object {
        const val STANDARD_ACCOUNT_START_INDEX = 0
    }
}
