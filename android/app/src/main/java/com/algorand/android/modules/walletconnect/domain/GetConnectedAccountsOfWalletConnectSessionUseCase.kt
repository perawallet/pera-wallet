...
class GetConnectedAccountsOfWalletConnectSessionUseCase @Inject constructor(
    ...
    suspend operator fun invoke(
        walletConnectSessionEntity: WalletConnectSessionEntity
    ): List<WalletConnectSessionAccountEntity> {
        return walletConnectRepository.getConnectedAccountsOfSession(walletConnectSessionEntity.id).orEmpty()
    }
}
